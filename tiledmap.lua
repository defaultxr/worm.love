...
function TiledMap_DrawTLCam (camx,camy,fun_layercallback)
    camx,camy = floor(camx),floor(camy)
    local screen_w = love.graphics.getWidth()
    local screen_h = love.graphics.getHeight()
    local minx,maxx = floor(camx/kTileSize),ceil((camx+screen_w)/kTileSize)
    local miny,maxy = floor(camy/kTileSize),ceil((camy+screen_h)/kTileSize)
    for z = 1,#gMapLayers do 
    if (fun_layercallback) then fun_layercallback(z,gMapLayers[z]) end
    if (TiledMap_IsLayerVisible(z)) then
    for x = minx,maxx do
    for y = miny,maxy do
        local gfx = gTileGfx[TiledMap_GetMapTile(x,y,z)]
        if (gfx) then
            local sx = x*kTileSize - camx
            local sy = y*kTileSize - camy
            love.graphics.draw(gfx,sx,sy) -- x, y, r, sx, sy, ox, oy
        end
    end
    end
